...
- Ese archivo se pega en un proyecto de Apps Script ligado a la planilla (`doGet`
  lo sirve). El registro llama a `google.script.run.registrarParticipante(...)`;
  la identidad es el **DNI** (no se duplica).
...
Paso a paso completo (crear la planilla, pegar el código, publicar):
**`docs/google-apps-script.md`**. El servidor está en **`apps-script/Codigo.gs`**.

Carpetas nuevas: `scripts/` (build), `apps-script/` (servidor), `docs/`
(instructivos), `dist/` (los HTML empaquetados, generados).
...
2. **Cuando toque publicar:** se empaqueta con `node scripts/empaquetar.js` y se
   sube el `Index.html` a Apps Script (ver `docs/google-apps-script.md`). La fuente
   modular sigue yendo a Drive como respaldo.
